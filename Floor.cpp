...
std::array<std::pair<bool, bool>, 3> Floor::calc_colision(const glm::vec3& other_pos)
{
	return std::array<std::pair<bool,bool>, 3>{
		std::pair<bool, bool>{false, false},
		std::pair<bool, bool>{
			other_pos.y <= m_position.y, false
		},
		std::pair<bool, bool>{false, false}
	};
}
...
void Floor::draw(const glm::mat4& baseM, ShaderProgram* sp)
{
	...
	for (int i = -range; i < range; i++) {
		for (int j = -range; j < range; j++) {
			...
			glVertexAttribPointer(sp->a("vertex"), 4, GL_FLOAT, false, 0, myCubeVertices); //Wska¿ tablicê z danymi dla atrybutu vertex

			glEnableVertexAttribArray(sp->a("color"));
			glVertexAttribPointer(sp->a("color"), 4, GL_FLOAT, false, 0, myCubeColors); // za³aduj kolory

			glDrawArrays(GL_TRIANGLES, 0, myCubeVertexCount); //Narysuj obiekt

			glDisableVertexAttribArray(sp->a("vertex"));  //Wy³¹cz przesy³anie danych do atrybutu vertex
			glDisableVertexAttribArray(sp->a("color"));  //Wy³¹cz przesy³anie danych do atrybutu vertex
		}
	}
}
