...
glm::mat4 CollidableObj::calc_base_M(const glm::mat4& baseM)
{
	...
	newM = glm::rotate(newM, glm::radians(m_rotation_per_axis.x), glm::vec3(1.f, 0.f, 0.f));
	newM = glm::rotate(newM, glm::radians(m_rotation_per_axis.y), glm::vec3(0.f, 1.f, 0.f));
	newM = glm::rotate(newM, glm::radians(m_rotation_per_axis.z), glm::vec3(0.f, 0.f, 1.f));
	
	return newM;
}

std::array<std::pair<bool, bool>, 3> CollidableObj::calc_colision(const glm::vec3& other_pos)
{
	...
	bool colided = diff.x >= -m_bounding_box_radius.x &&
	...
	std::cout << "\t\tcollision X:" << collision_array[0].first << ", " << collision_array[0].second << std::endl;
	std::cout << "\t\tcollision Y:" << collision_array[1].first << ", " << collision_array[1].second << std::endl;
	std::cout << "\t\tcollision Z:" << collision_array[2].first << ", " << collision_array[2].second << std::endl;
