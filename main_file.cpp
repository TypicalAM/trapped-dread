...
void keyCallback(GLFWwindow* window,int key,int scancode,int action,int mods) {
    if (action==GLFW_PRESS) {
        if (key==GLFW_KEY_W) movement_mask		|= MOVE_FORWARD;
        if (key==GLFW_KEY_S) movement_mask		|= MOVE_BACKWARD;
        if (key==GLFW_KEY_A) movement_mask		|= MOVE_LEFT; 
        if (key==GLFW_KEY_D) movement_mask		|= MOVE_RIGHT; 
		if (key==GLFW_KEY_SPACE) movement_mask  |= MOVE_UP;
		if (key==GLFW_KEY_LEFT_CONTROL) 
								  movement_mask |= MOVE_DOWN;
		if (key == GLFW_KEY_T) camera_ptr->toggle_fly();
		if (key == GLFW_KEY_EQUAL) camera_ptr->inc_pitch(); //toggle ala noclip
		if (key == GLFW_KEY_KP_SUBTRACT) camera_ptr->inc_yaw(); //toggle ala noclip
		if (key == GLFW_KEY_0) camera_ptr->reset_cam(); //toggle ala noclip
    ...
    if (action==GLFW_RELEASE) {
        ...
        if (key==GLFW_KEY_D) movement_mask &= ~MOVE_RIGHT;
		if (key == GLFW_KEY_SPACE) movement_mask &= ~MOVE_UP;
		if (key == GLFW_KEY_LEFT_CONTROL) movement_mask &= ~MOVE_DOWN;
	}
}
...
void mouse_move_callback(GLFWwindow* window, double xpos, double ypos) {
	...
	glfwSetCursorPos(window, static_cast<double>(INITAL_WIDTH/ 2), static_cast<double>(INITIAL_HEIGHT / 2));
...
void setupInitialPositionsOfObjects() {
	...
	GameObjects.push_back(
		std::unique_ptr<Floor>(new Floor(0.4f))
	);
...
int main(void)
{
	...
	window = glfwCreateWindow(INITAL_WIDTH, INITIAL_HEIGHT, "OpenGL", NULL, NULL);  //Utwórz okno 500x500 o tytule "OpenGL" i kontekst OpenGL.
