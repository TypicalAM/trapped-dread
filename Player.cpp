...
void Player::mouse_move_callback(double xpos, double ypos) {
	...
	float delta_horizontal =  static_cast<float>(xpos - static_cast<double>(m_screen_width / 2));
	m_euler_angles.y += delta_horizontal * m_sensitivity; //TODO czemu -= ??

	float delta_vertical =  static_cast<float>(ypos - static_cast<double>(m_sceen_height / 2));
...
{
	glm::vec3 forwards{
		cos(glm::radians(m_euler_angles.y)) * cos(glm::radians(m_euler_angles.x)),
		sin(glm::radians(m_euler_angles.x)),
		sin(glm::radians(m_euler_angles.y)) * cos(glm::radians(m_euler_angles.x))
	};

	return forwards;
}

void Player::update(double timeDelta, int movement_mask, float speed, const std::vector<std::unique_ptr<CollidableObj>> &other_objs)
{
	// constat for gravity
	...
	for (auto &other_obj : other_objs) {
		...
		std::cout << "other object type: " << typeid(*other_obj).name() << std::endl;

		std::array <std::pair<bool, bool>, 3> collisions = other_obj->calc_colision(m_cam_pos);
