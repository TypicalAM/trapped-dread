...
#define INITIAL_CAM_POS glm::vec3(0.f, 0.f, -4.f)
...
public:
	Player(float screen_width, float screen_height) {
	...
	void setFloorLevel(float x) {
		base_floor_level = x;
	}

	void moveCam(const glm::vec3& offset) {
		m_cam_pos += offset;
	}
	...
	void update(double timeDelta, int movement_mask, float speed, const std::vector<std::unique_ptr<CollidableObj>> &other_objs);
